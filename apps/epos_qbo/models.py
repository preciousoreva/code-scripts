...
class InventoryReviewItem(models.Model):
    REVIEW_OPEN = "open"
    REVIEW_ACKNOWLEDGED = "acknowledged"
    REVIEW_IGNORED = "ignored"
    REVIEW_MANUALLY_RESOLVED = "manually_resolved"
    REVIEW_RESOLVED_BY_RERUN = "resolved_by_rerun"
    REVIEW_STATUS_CHOICES = [
        (REVIEW_OPEN, "Open"),
        (REVIEW_ACKNOWLEDGED, "Acknowledged"),
        (REVIEW_IGNORED, "Ignored"),
        (REVIEW_MANUALLY_RESOLVED, "Manually resolved"),
        (REVIEW_RESOLVED_BY_RERUN, "Resolved by rerun"),
    ]
    ...
    run_job = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="inventory_review_items",
    )
    ...
    reason_group = models.CharField(max_length=64, blank=True, db_index=True)
    epos_expected_qty = models.DecimalField(max_digits=19, decimal_places=4, null=True, blank=True)
    qbo_qty = models.DecimalField(max_digits=19, decimal_places=4, null=True, blank=True)
    delta = models.DecimalField(max_digits=19, decimal_places=4, null=True, blank=True)
    source_row_json = models.JSONField(default=dict, blank=True)
    ...
    resolution_type = models.CharField(max_length=64, blank=True)
    ...
    resolved_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="resolved_inventory_review_items",
    )
    resolved_at = models.DateTimeField(null=True, blank=True)

    first_seen_at = models.DateTimeField(default=timezone.now)
    last_seen_at = models.DateTimeField(default=timezone.now, db_index=True)
    last_seen_run_label = models.CharField(max_length=64, blank=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["company_key", "normalized_product_name", "reason_group", "status_code"],
                name="uniq_inventory_review_item_key",
            )
        ]
        indexes = [
            models.Index(fields=["company_key", "is_active", "review_status"], name="inv_review_company_active_idx"),
            models.Index(fields=["company_key", "reason_group", "is_active"], name="inv_review_company_reason_idx"),
            models.Index(fields=["company_key", "normalized_product_name"], name="inv_review_company_norm_idx"),
        ]
        ordering = ["-last_seen_at", "-occurrence_count", "product_name"]

    def __str__(self) -> str:
        return f"{self.company_key}: {self.product_name} ({self.reason_group})"
