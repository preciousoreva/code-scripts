...
class CatalogCleanupBuildCommandTests(TestCase):
    def _base_cleaned(self, **overrides) -> dict:
        cleaned = {
    ...
    def test_minimum_required_args_emits_auto_download(self):
        ...
        flat = " ".join(cmd)
        self.assertIn("-m", cmd)
        self.assertIn("code_scripts.inventory_catalog_cleanup", cmd)
        self.assertIn("--company", cmd)
        self.assertIn("company_a", cmd)
        self.assertIn("--auto-download", cmd)
        self.assertIn("--auto-fetch-qbo", cmd)
    ...
    def test_category_product_and_preview_mode_become_cli_args(self):
        cmd = build_command(
            self._base_cleaned(
                inventory_options={
                    "categories": ["ALCOHOLS & SPIRITS"],
                    "product_filter": "ROOTZ",
                    "dry_run": True,
                    "max_products": 2,
                }
            )
