...
class CatalogCleanupTriggerViewTests(TestCase):
    ...
    def test_requires_login(self):
        response = self.client.post(reverse("epos_qbo:run-trigger-catalog-cleanup"))
        self.assertEqual(response.status_code, 302)
    ...
    def test_requires_permission(self):
        ...
        self.client.login(username="np", password="pw")
        response = self.client.post(
            reverse("epos_qbo:run-trigger-catalog-cleanup"),
            {"company_key": "company_a"},
        )
        self.assertEqual(response.status_code, 403)

    def test_plan_only_creates_job_without_apply_or_dry_run(self):
        self.client.login(username="op", password="pw")
        with mock.patch("apps.epos_qbo.views.dispatch_next_queued_job", return_value=(None, "queued")):
            response = self.client.post(
                reverse("epos_qbo:run-trigger-catalog-cleanup"),
                {
                    "company_key": "company_a",
        ...
        self.assertEqual(response.status_code, 302)
        job = RunJob.objects.get()
        self.assertEqual(job.scope, RunJob.SCOPE_INVENTORY_CATALOG_CLEANUP)
        self.assertEqual(job.company_key, "company_a")
        self.assertEqual(job.inventory_options_json.get("categories"), ["Beverages"])
        self.assertEqual(job.inventory_options_json.get("product_filter"), "Widget")
        self.assertNotIn("dry_run", job.inventory_options_json)
        self.assertNotIn("apply", job.inventory_options_json)
        self.assertNotIn("max_products", job.inventory_options_json)

    def test_preview_defaults_max_products_to_1_and_sets_dry_run(self):
        self.client.login(username="op", password="pw")
        with mock.patch("apps.epos_qbo.views.dispatch_next_queued_job", return_value=(None, "queued")):
            response = self.client.post(
                reverse("epos_qbo:run-trigger-catalog-cleanup"),
        ...
        self.assertEqual(job.scope, RunJob.SCOPE_INVENTORY_CATALOG_CLEANUP)
        self.assertTrue(job.inventory_options_json.get("dry_run"))
        self.assertEqual(job.inventory_options_json.get("max_products"), 1)
    ...
    def test_apply_creates_job_with_apply_flag_and_max_products(self):
        self.client.login(username="op", password="pw")
        with mock.patch("apps.epos_qbo.views.dispatch_next_queued_job", return_value=(None, "queued")):
            response = self.client.post(
                reverse("epos_qbo:run-trigger-catalog-cleanup"),
        ...
        job = RunJob.objects.get()
        self.assertEqual(job.scope, RunJob.SCOPE_INVENTORY_CATALOG_CLEANUP)
        self.assertTrue(job.inventory_options_json.get("apply"))
        self.assertEqual(job.inventory_options_json.get("max_products"), 3)
